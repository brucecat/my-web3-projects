...
function Layout({
  ...
  return (
    <div className="flex h-screen">
      <nav
        className={`hidden w-0 p-0 md:p-4 md:w-[300px] md:flex md:flex-col md:justify-between ${menuVisible === true
      ...
      >
        ...
        <ul className="flex flex-col gap-2 py-2 md:gap-6">
          {allDemoList.map(({ name, description, url, technologyStack, icon }) => (
            <Link key={name} href={url}>
              <li
                className={`p-3 rounded-md cursor-pointer hover:bg-gray-100 hover:text-black flex items-center gap-2 ${router.route === url
