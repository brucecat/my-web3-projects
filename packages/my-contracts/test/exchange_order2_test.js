...
module.exports = async function (callback) {
  ...
  try {
    // 创建订单
    for (let i = 1; i <= 5; i++) {
      const orderParamList = [tokenInstance.address, toWei(100 + i), ETHER_ADDRESS, toWei(i / 1000)]
      console.log('orderParamList: ', orderParamList);
      await exchangeInstance.makeOrder(...orderParamList, {
        from: accounts[0]
      })
    ...
    for (let i = 1; i <= 5; i++) {
      await exchangeInstance.makeOrder(tokenInstance.address, toWei(100 + i), ETHER_ADDRESS, toWei(i / 1000), {
        from: accounts[1]
      })
